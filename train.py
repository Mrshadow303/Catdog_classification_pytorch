...
TRAIN_DIR = DATA_DIR + "/new_catdog_train_set"  # 训练集路径
TEST_DIR = DATA_DIR + "/new_catdog_test_set"  # 测试集路径
BATCH_SIZE = 32  # 一次训练的样本量
NUM_WORKERS = 0  # 有多少子进程将用于数据的加载，若使用 Windows 建议设置为 0
NUM_EPOCHS = 1  # 训练轮数
LEARNING_RATE = 1e-3  # 训练学习率
CONTINUE_TRAIN = False  # 是否使用以前的 pth 文件继续训练？
PTH_FILE = f"{DATA_DIR}/checkpoint_80.3571%.pth"  # 调用之前生成好的 pth 文件并继续训练
...
def train(
    ...
    :param optimizer: 优化器
    :param loss_function: 损失函数
    :return: dict{ 训练平均损失, 训练准确度, , 测试准确度 测试平均损失}
    """
    global BEST_ACC
    # 创建空结果字典，用于后期 plt 绘图
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        if test_acc > BEST_ACC:
            ...
            torch.save(
                model.state_dict(),
                f"{PTH_SAVE_DIR}/Catdogcheckpoint_{test_acc*100:.4f}%.pth",
            )
